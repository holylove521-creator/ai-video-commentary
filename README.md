...
AI 智能剪辑不只是挑选精彩片段——更要确保剪出来的视频**逻辑通顺、叙事完整**。系统内置四层连贯性保障：

### 四层保障说明

| 层级 | 模块 | 作用 |
|------|------|------|
| ① 叙事分析 | `NarrativeAnalyzer.analyze_narrative()` | 识别核心叙事主线、将场景分类为"必要 / 增色 / 可删"、标记四幕结构与转折点 |
| ② 因果链保护 | `select_scenes_by_narrative()` | 若保留了某个"结果"场景，其对应的"原因"场景将被强制保留，防止逻辑断层 |
| ③ 过渡解说 | `inject_transitions()` | 在跳跃超过 3 秒的场景间自动生成 10-20 字的衔接解说词，消除剪辑突兀感 |
| ④ 评分反馈 | `coherence_check()` | 从时间线清晰度、因果逻辑、人物连续性、信息完整性四个维度打分（0-10），低于阈值时输出具体问题列表 |
...
└─────────────────┬───────────────────────┘
                  ...
                  ▼
┌─────────────────────────────────────────┐
│    Stage 2a: 叙事分析（NarrativeAnalyzer）│
│  ① 叙事主线识别 → ② 场景分类          │
│  ③ 因果链保护  → ④ 连贯性优先筛选     │
└─────────────────┬───────────────────────┘
                  │ 筛选后场景列表
                  ▼
┌─────────────────────────────────────────┐
│    Stage 2b: 解说脚本生成（ScriptGenerator）│
│  LLM 脚本创作 → 过渡解说注入 → 评分反馈 │
│  (Qwen2.5-32B via llama-server)         │
└─────────────────┬───────────────────────┘
                  │ 结构化脚本 [{start,end,text,emotion}]
                  ▼
...
| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `--target-duration` | float | `0` | 目标视频时长（秒）。`0` 表示保留所有必要场景，不裁剪时长；设置正数后，系统在保证叙事骨架完整的前提下，用"增色"场景填充至目标时长 |
| `--coherence-threshold` | float | `7.0` | 连贯性评分阈值（0-10）。最终脚本评分低于此值时，日志会输出具体问题列表供参考 |

```bash
# 示例：生成 2 分钟精华剪辑，连贯性阈值设为 8 分
python main.py --input movie.mp4 --style doc \
    --target-duration 120 \
    --coherence-threshold 8.0
```
